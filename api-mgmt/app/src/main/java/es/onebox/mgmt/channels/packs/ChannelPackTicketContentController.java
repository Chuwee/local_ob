...
@Validated
@RestController
@RequestMapping(value = ChannelPackTicketContentController.BASE_URI)
public class ChannelPackTicketContentController {

    public static final String BASE_URI = ApiConfig.BASE_URL + "/channels/{channelId}/packs/{packId}/ticket-contents";
    ...
    private static final String AUDIT_COLLECTION = "CHANNEL_PACK_TICKET_CONTENTS";
    private static final String AUDIT_SUBCOLLECTION_PDF = "PDF";
    private static final String AUDIT_SUBCOLLECTION_PRINTER = "PRINTER";

    private final ChannelPacksContentsService packsContentsService;

    @Autowired
    public ChannelPackTicketContentController(ChannelPacksContentsService packsContentsService) {
        this.packsContentsService = packsContentsService;
    }

    @Secured({ROLE_EVN_MGR, ROLE_ENT_ANS, ROLE_OPR_MGR, ROLE_OPR_ANS})
    @RequestMapping(method = RequestMethod.GET, value = "/PDF/texts")
    public PackTicketContentsTextListDTO getTicketsPDFTextContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
        ...
        return packsContentsService.getPackTicketContentsTexts(channelId, packId, filter, TicketCommunicationElementCategory.PDF);
    }

    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Secured({ROLE_EVN_MGR, ROLE_OPR_MGR})
    @RequestMapping(method = RequestMethod.POST, value = "/PDF/texts", produces = MediaType.APPLICATION_JSON_VALUE)
    public void updateTicketPDFTextContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                            @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
        ...
        packsContentsService.updatePackTicketContentsTexts(channelId, packId, contents, TicketCommunicationElementCategory.PDF);
    }

    @Secured({ROLE_EVN_MGR, ROLE_ENT_ANS, ROLE_OPR_MGR, ROLE_OPR_ANS})
    @RequestMapping(method = RequestMethod.GET, value = "/PDF/images")
    public PackTicketContentsImagePDFListDTO getTicketsPDFImageContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                                                        @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
                                                                        @BindUsingJackson @Valid PackTicketContentImagePDFFilter filter) {
        Audit.addTags(AUDIT_SERVICE, AUDIT_COLLECTION, AUDIT_SUBCOLLECTION_PDF, AuditTag.AUDIT_ACTION_GET);
        return packsContentsService.getPackTicketContentsPDFImages(channelId, packId, filter, TicketCommunicationElementCategory.PDF);
    }

    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Secured({ROLE_EVN_MGR, ROLE_OPR_MGR})
    @RequestMapping(method = RequestMethod.POST, value = "/PDF/images", produces = MediaType.APPLICATION_JSON_VALUE)
    public void updateTicketPDFImageContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                             @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
                                             @Valid @NotEmpty @RequestBody PackTicketContentsImagePDFListDTO contents) {
        Audit.addTags(AUDIT_SERVICE, AUDIT_COLLECTION, AUDIT_SUBCOLLECTION_PDF, AuditTag.AUDIT_ACTION_UPDATE);
        packsContentsService.updatePackTicketContentsPDFImages(channelId, packId, contents, TicketCommunicationElementCategory.PDF);
    }

    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Secured({ROLE_EVN_MGR, ROLE_OPR_MGR})
    @RequestMapping(method = RequestMethod.DELETE, value = "/PDF/images/languages/{language}/types/{type}")
    public void deleteTicketPDFContentsImage(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                             @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
        ...
        packsContentsService.deletePackTicketContentPDFImage(channelId, packId, language, type, TicketCommunicationElementCategory.PDF);
    }

    @Secured({ROLE_EVN_MGR, ROLE_ENT_ANS, ROLE_OPR_MGR, ROLE_OPR_ANS})
    @RequestMapping(method = RequestMethod.GET, value = "/PRINTER/texts")
    public PackTicketContentsTextListDTO getTicketsPrinterTextContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                                                       @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
                                                                       @BindUsingJackson @Valid PackTicketContentTextPDFFilter filter) {
        Audit.addTags(AUDIT_SERVICE, AUDIT_COLLECTION, AUDIT_SUBCOLLECTION_PRINTER, AuditTag.AUDIT_ACTION_GET);
        return packsContentsService.getPackTicketContentsTexts(channelId, packId, filter, TicketCommunicationElementCategory.TICKET_OFFICE);
    }

    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Secured({ROLE_EVN_MGR, ROLE_OPR_MGR})
    @RequestMapping(method = RequestMethod.POST, value = "/PRINTER/texts", produces = MediaType.APPLICATION_JSON_VALUE)
    public void updateTicketPrintersTextContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                                 @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
                                                 @Valid @NotEmpty @RequestBody PackTicketContentsTextListDTO contents) {
        Audit.addTags(AUDIT_SERVICE, AUDIT_COLLECTION, AUDIT_SUBCOLLECTION_PRINTER, AuditTag.AUDIT_ACTION_UPDATE);
        packsContentsService.updatePackTicketContentsTexts(channelId, packId, contents, TicketCommunicationElementCategory.TICKET_OFFICE);
    }

    @Secured({ROLE_EVN_MGR, ROLE_ENT_ANS, ROLE_OPR_MGR, ROLE_OPR_ANS})
    @RequestMapping(method = RequestMethod.GET, value = "/PRINTER/images")
    public PackTicketContentsImagePrinterListDTO getTicketsPrinterImageContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                                                                @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
                                                                                @BindUsingJackson @Valid PackTicketContentImagePrinterFilter filter) {
        Audit.addTags(AUDIT_SERVICE, AUDIT_COLLECTION, AUDIT_SUBCOLLECTION_PRINTER, AuditTag.AUDIT_ACTION_GET);
        return packsContentsService.getPackTicketContentsPrinterImages(channelId, packId, filter, TicketCommunicationElementCategory.TICKET_OFFICE);
    }

    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Secured({ROLE_EVN_MGR, ROLE_OPR_MGR})
    @RequestMapping(method = RequestMethod.POST, value = "/PRINTER/images", produces = MediaType.APPLICATION_JSON_VALUE)
    public void updateTicketPrinterImageContents(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                                 @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
                                                 @Valid @NotEmpty @RequestBody PackTicketContentsImagePrinterListDTO contents) {
        Audit.addTags(AUDIT_SERVICE, AUDIT_COLLECTION, AUDIT_SUBCOLLECTION_PRINTER, AuditTag.AUDIT_ACTION_UPDATE);
        packsContentsService.updatePackTicketContentsPrinterImages(channelId, packId, contents, TicketCommunicationElementCategory.TICKET_OFFICE);
    }

    @ResponseStatus(HttpStatus.NO_CONTENT)
    @Secured({ROLE_EVN_MGR, ROLE_OPR_MGR})
    @RequestMapping(method = RequestMethod.DELETE, value = "/PRINTER/images/languages/{language}/types/{type}")
    public void deleteTicketPrinterContentsImage(@PathVariable @Min(value = 1, message = CHANNEL_ID_MUST_BE_ABOVE_0) Long channelId,
                                                 @PathVariable @Min(value = 1, message = PACK_ID_MUST_BE_ABOVE_0) Long packId,
        ...
        packsContentsService.deletePackTicketContentPrinterImage(channelId, packId, language, type, TicketCommunicationElementCategory.TICKET_OFFICE);
    }
}
