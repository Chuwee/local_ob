...
export const productSalesDetailsResolver: ResolveFn<TicketDetail> = (route: ActivatedRouteSnapshot) => {
    ...
    return combineLatest([
        ...
        mergeMap(([ticketDetail, error]) => {
            if (error) {
                router.navigate(['/products-sales']);
                return EMPTY;
            }
            ...
            if (route?.data?.['breadcrumb']) {
                breadcrumbsSrv.addDynamicSegment(route.data['breadcrumb'],
                    translate.instant('TITLES.PRODUCT', { ticketId: ticketDetail.id }));
            }
