import { packSaleRequestStatus } from '@admin-clients/cpanel-channels-packs-sale-requests-data-access';
...
@Component({
    selector: 'app-packs-sale-requests-list-filter',
    imports: [
    ...
    templateUrl: './packs-sale-requests-list-filter.component.html',
    styleUrls: ['./packs-sale-requests-list-filter.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush
})
export class PacksSaleRequestsListFilterComponent extends FilterWrapped {
    ...
    removeFilter(key: string, value: unknown): void {
        if (key === 'START_DATE') {
            ...
            const statusCheck = packSaleRequestStatus.find(status => status === value);
    ...
    applyFiltersByUrlParams$(params: Params): Observable<FilterItem[]> {
        ...
        if (params['status']) {
            params['status'].split(',').forEach((statusValue: string) => {
                const statusKey = packSaleRequestStatus.find(status => status === statusValue);
                if (statusKey) {
    ...
    private getFilterStartDate(): FilterItem {
        const filterItem = new FilterItem('START_DATE', this.#translate.instant('PACKS_SALE_REQUESTS.FORMS.LABELS.DATE_FROM'));
    ...
    private getFilterEndDate(): FilterItem {
        const filterItem = new FilterItem('END_DATE', this.#translate.instant('PACKS_SALE_REQUESTS.FORMS.LABELS.DATE_TO'));
    ...
    private getFilterStatus(): FilterItem {
        const filterItem = new FilterItem('STATUS', this.#translate.instant('PACKS_SALE_REQUESTS.FORMS.LABELS.STATUS'));
        ...
        if (mappedValues.length > 0) {
            filterItem.values = mappedValues.map(statusCheck => new FilterItemValue(
                ...
                this.#translate.instant(`PACKS_SALE_REQUESTS.FORMS.STATUS_OPTS.${statusCheck.toUpperCase()}`)
            ));
