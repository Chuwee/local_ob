...
import { ChannelType, ChannelsExtendedService, ChannelsService, channelWebTypes } from '@admin-clients/cpanel/channels/data-access';
...
@Component({
    selector: 'app-channel-communication-booking-confirm',
    templateUrl: './channel-communication-booking-confirm.component.html',
    styleUrls: ['./channel-communication-booking-confirm.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
    standalone: false
})
export class ChannelCommunicationBookingConfirmComponent implements OnInit, OnDestroy, WritingComponent {
    ...
    ngOnInit(): void {
        ...
        this.selectedLanguage$
            ...
            .subscribe(contents => {
                ...
                const bookingCheckout = contents.find(content => content.id === this._contentsIds.bookingCheckout);
                ...
                this.placeholdersMap = {
                    ...
                    bookingCheckout: bookingCheckout?.labels?.map(({ code }) => code),
                    footer: footer.labels?.map(({ code }) => code)
                };
                this.form.patchValue({
                    subject: header?.subject,
                    header: header?.value,
                    bookingCheckout: bookingCheckout?.value,
                    footer: footer?.value
                });
    ...
    save$(): Observable<void> {
        if (this.form.valid) {
            ...
            const bookingCheckout: ChannelContent = {
                id: this._contentsIds.bookingCheckout,
                language: currentLang,
                value: formValues.bookingCheckout
            };
            ...
            if (formValues.bookingCheckout != null) {
                data.push(bookingCheckout);
            }
