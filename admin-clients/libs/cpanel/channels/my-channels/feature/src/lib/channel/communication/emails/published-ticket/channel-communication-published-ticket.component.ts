...
@Component({
    selector: 'app-channel-communication-published-ticket',
    templateUrl: './channel-communication-published-ticket.component.html',
    styleUrls: ['./channel-communication-published-ticket.component.scss'],
...
export class ChannelCommunicationPublishedTicketComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        this.selectedLanguage$
            ...
            .subscribe(contents => {
                ...
                const soldInfo = contents.find(content => content.id === this.#contentsIds.soldInfo);

                this.placeholdersMap = {
                    header: header?.labels?.map(({ code }) => code),
                    soldInfo: soldInfo?.labels?.map(({ code }) => code),
    ...
    save$(): Observable<void> {
        if (this.form.valid) {
            ...
            const soldInfo: ChannelContent = {
                id: this.#contentsIds.soldInfo,
                language: currentLang,
                value: formValues.soldInfo
            };
            const data = [header, footer, soldInfo];
