...
export class PackTicketContentPrinterComponent implements OnInit, AfterViewInit {
    ...
    cancel(): void {
        ...
        this.#packsSrv.packPrinterTexts.load(this.$pack().id);
        this.#packsSrv.packPrinterImages.load(this.$pack().id);
    }
    ...
    save(
        ...
        if (textsToSave.length > 0) {
            obsToSave$.push(
                this.#packsSrv.packPrinterTexts.save(this.$pack().id, textsToSave)
            );
        }
        ...
        if (imagesToSave.length > 0) {
            obsToSave$.push(
                this.#packsSrv.packPrinterImages.save(this.$pack().id, imagesToSave)
            );
        }

        if (imagesToDelete.length > 0) {
            obsToSave$.push(
                this.#packsSrv.packPrinterImages.delete(this.$pack().id, imagesToDelete)
            );
        }
