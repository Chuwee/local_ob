...
export class PackTicketContentPdfComponent implements OnInit, AfterViewInit {
    ...
    cancel(): void {
        ...
        this.#packsSrv.packTicketTexts.load(this.$pack().id);
        this.#packsSrv.packTicketImages.load(this.$pack().id);
    }
    ...
    save(
        ...
        if (textsToSave.length > 0) {
            obsToSave$.push(this.#packsSrv.packTicketTexts.save(this.$pack().id, textsToSave));
        }

        const { imagesToSave, imagesToDelete } = getImageFields(this.pdfContentForm, this.#imageFields, this.#selectedLanguage);
        if (imagesToSave.length > 0) {
            obsToSave$.push(this.#packsSrv.packTicketImages.save(this.$pack().id, imagesToSave));
        }

        if (imagesToDelete.length > 0) {
            obsToSave$.push(this.#packsSrv.packTicketImages.delete(
                this.$pack().id, imagesToDelete
            ));
        }
