...
import { PackPrice, PackRate, PacksService, PutPackPrice, VmPackPrice } from '@admin-clients/cpanel/packs/my-packs/data-access';
...
import { FormGroup, ReactiveFormsModule, UntypedFormArray, UntypedFormBuilder, UntypedFormControl, Validators } from '@angular/forms';
...
export class PackPriceTypesMatrixComponent implements OnInit, OnDestroy {
        ...
        if (!packPrices.length) {
            priceTypes.forEach(priceType => {
                const newPrice = {
                    ...
                    rate: {
                        id: priceType.rate?.id ?? null,
                        name: priceType.rate?.name ?? this.$pack().name
                    },
                    value: 0
                };
    ...
    #createRate(modifiedPrices: PutPackPrice[]): Observable<void> {
        ...
        if (modifiedPrices[0].rate_id === null) {
            ...
            return this.#packsSrv.packRates.create(this.$pack().id, rate)
    ...
    #save(modifiedPrices: PutPackPrice[]): Observable<void> {
        return this.#packsSrv.packPrices.update(this.$pack().id, modifiedPrices)
            .pipe(
