...
@Repository
public class RangeSurchargeEventInvitationDao extends DaoImpl<CpanelRangoRecargoEventoInvRecord, Integer> {

    protected RangeSurchargeEventInvitationDao() {
    ...
    public List<CpanelRangoRecord> getByEventId(int eventoId) {
        return dsl.select(CPANEL_RANGO.fields())
                .from(CPANEL_RANGO)
                .innerJoin(CPANEL_RANGO_RECARGO_EVENTO_INV)
                .on(CPANEL_RANGO.IDRANGO.eq(Tables.CPANEL_RANGO_RECARGO_EVENTO_INV.IDRANGO))
                .where(CPANEL_RANGO_RECARGO_EVENTO_INV.IDEVENTO.eq(eventoId))
    ...
    public int deleteByEventId(int eventId) {
        return this.dsl.deleteFrom(CPANEL_RANGO_RECARGO_EVENTO_INV)
                .where(CPANEL_RANGO_RECARGO_EVENTO_INV.IDEVENTO.eq(eventId))
                .execute();
    }
}